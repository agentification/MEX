...
class OneDTransitionRewardModel(Model):
    ...
    def update(
        ...
        obs, action_replay, _, gt_reward, _ = batch.astuple()
        obs = model_util.to_tensor(obs).to(self.device)
        action, _, _ = agent.sac_agent.policy.sample(obs.float())
        # Model step
        model_in = self._get_model_input(obs, action)
        means, logvars = self.forward(model_in)
        variances = logvars.exp()
        stds = torch.sqrt(variances)
        gauss = Normal(means, stds)
        preds = torch.normal(means, stds)
        log_prob = gauss.log_prob(preds)
        ...
        if self.target_is_delta:
            ...
            next_obs = tmp_
        pred_reward = preds[:, -1:] if self.learned_rewards else gt_reward

        gamma = 0.99
        next_state_action, _, _ = agent.sac_agent.policy.sample(next_obs.float())
        qf1, qf2 = agent.sac_agent.critic(next_obs.float(), next_state_action.float())
        critic_value = pred_reward + gamma * torch.min(qf1, qf2).detach()
        qf1_b, qf2_b = agent.sac_agent.critic(obs.float(), action.float())
        baseline_value = torch.min(qf1_b, qf2_b).detach()
        critic_value -= baseline_value
        critic_value_std = critic_value.std()
        if not critic_value_std.isnan():
            critic_value = (critic_value - critic_value.mean()) / (critic_value_std + 1e-8).detach()
        loss += -(log_prob * critic_value).mean() * coeff
        loss.backward()
        if meta is not None:
            with torch.no_grad():
                grad_norm = 0.0
